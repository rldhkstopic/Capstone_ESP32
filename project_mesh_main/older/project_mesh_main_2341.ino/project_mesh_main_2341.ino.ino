...
void preproNode(String &Msg){
  String nodeMsg = Msg;
   ...
   if(nodeMsg.indexOf("ECEIVED::")>0){
    ...
    Serial.println(NODENAME_from.substring(1));
    switch((NODENAME_from.substring(1).toInt())){
      case 1: // N1
          N1[0] = 0;
          if(NODENAME_1st!="")
            if(nt(NODENAME_1st.substring(1))<NODECNT+1) 
              N1[(NODENAME_1st.substring(1))] = (NODERSSI_1st)*-1;
          if(NODENAME_2nd!="")
            if(t(NODENAME_2nd.substring(1))<NODECNT+1) 
              N1[(NODENAME_2nd.substring(1))] = (NODERSSI_2nd)*-1;
          if(NODENAME_3nd!="")
            if((NODENAME_3nd.substring(1))<NODECNT+1) 
              N1[nt(NODENAME_3nd.substring(1))] = (NODERSSI_3nd)*-1;
          if(NODENAME_4nd!="")
            if((NODENAME_4nd.substring(1))<NODECNT+1) 
              N1[(NODENAME_4nd.substring(1))] = (NODERSSI_4nd)*-1;
        break;
      case 2: // N2
          N2[2] = 0;
          if(NODENAME_1st!="")
            if((NODENAME_1st.substring(1))<NODECNT+1) 
              N2[(NODENAME_1st.substring(1).toInt())] = (NODERSSI_1st).toInt()*-1;
          if(NODENAME_2nd!="")
            if((NODENAME_2nd.substring(1))<NODECNT+1) 
    ...
    for(int i=0;i<NODECNT;i++){Serial.print(N1[i]);Serial.print(" ");} Serial.println();
    for(int i=0;i<NODECNT;i++){Serial.print(N2[i]);Serial.print(" ");} Serial.println();
    for(int i=0;i<NODECNT;i++){Serial.print(N3[i]);Serial.print(" ");} Serial.println();
    for(int i=0;i<NODECNT;i++){Serial.print(N4[i]);Serial.print(" ");} Serial.println();
    for(int i=0;i<NODECNT;i++){Serial.print(N5[i]);Serial.print(" ");} Serial.println();
}
